...
import {checkXgpuCloudAccess} from '../utils/xgpuAccess';
...
function TitleDetail({navigation, route}) {
  ...
  const [checkingXgpu, setCheckingXgpu] = React.useState(false);
  ...
  const handleStartGame = async () => {
    ...
    setCheckingXgpu(true);
    const titleId = titleItem.titleId || titleItem.XCloudTitleId;
    log.info('HandleStartCloudGame titleId:', titleId);
    try {
      const hasXgpuAccess = await checkXgpuCloudAccess(streamingTokens);
      if (!hasXgpuAccess) {
        Alert.alert(t('Warning'), t('NoXGP'));
        return;
      }
  ...
  return (
    <View style={styles.container}>
      <Spinner loading={!titleItem || checkingXgpu} text={t('Loading...')} />
...
const styles = StyleSheet.create({
  ...
  tvActionButtonFocused: {
    ...
    backgroundColor: 'rgba(220, 255, 255, 0.16)',
