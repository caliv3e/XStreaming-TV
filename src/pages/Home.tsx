...
function HomeScreen({navigation, route}) {
  ...
  const xgpuAccessChecking = React.useRef(false);
  ...
  const handleOpenCloud = async () => {
    if (xgpuAccessChecking.current) {
      return;
    }

    xgpuAccessChecking.current = true;
    setLoading(true);
    setLoadingText(t('Checking login status...'));
    ...
    try {
      ...
      log.info('checkXgpuCloudAccess error:', e);
      Alert.alert(t('Warning'), t('NoXGP'));
    } finally {
      xgpuAccessChecking.current = false;
      setLoading(false);
      setLoadingText('');
    }
  };
