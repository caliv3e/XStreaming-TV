...
export default class XcloudApi {
  ...
  probeCloudPlayAccess(consoleId: string, resolution: number): Promise<boolean> {
    log.info('[probeCloudPlayAccess] consoleId:', consoleId);
    if (!consoleId) {
    ...
    return new Promise<boolean>(resolve => {
      ...
      this.authedPost(`${this.host}/v5/sessions/cloud/play`, body, {
        timeout: 6 * 1000,
        headers: {
          'Content-Type': 'application/json',
          'X-MS-Device-Info': deviceInfo,
        ...
        .then(res => {
          ...
          const sessionPath = res.data?.sessionPath;
          const sessionId =
            typeof sessionPath === 'string' ? sessionPath.split('/')[3] : '';
          if (!sessionId) {
            resolve(true);
            return;
          }

          this.authedDelete(`${this.host}/v5/sessions/cloud/${sessionId}`, {
            timeout: 3 * 1000,
            headers: {
              'Content-Type': 'application/json',
            },
            ...
            .catch(error => {
              log.info(
                '[probeCloudPlayAccess] stop probe session error:',
